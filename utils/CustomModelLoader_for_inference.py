...
from utils.CustomAttnProcessor_for_inference import CustomDiffusionAttnProcessor

"""This is a modified version of the CustomModelLoader.py file. We allow the weights to contain either to_k or to_v"""
...
class CustomModelLoader(UNet2DConditionLoadersMixin):
    ...
    def load_attn_procs(self, pretrained_model_name_or_path_or_dict: Union[str, Dict[str, torch.Tensor]], train_q, train_k, train_v, train_out, controller,**kwargs):
        r"""
            ...
            for key, value_dict in custom_diffusion_grouped_dict.items():
                if len(value_dict) == 0:
                    attn_processors[key] = CustomDiffusionAttnProcessor(
                        train_k=False, train_v=False, train_q=False, train_out=False, hidden_size=None, cross_attention_dim=None, controller=None, place_in_unet=None
                    )
                else:
                    if "to_v_custom_diffusion.weight" in value_dict:
                    ...
                    elif "to_k_custom_diffusion.weight" in value_dict:
                        cross_attention_dim = value_dict["to_k_custom_diffusion.weight"].shape[1]
                        hidden_size = value_dict["to_k_custom_diffusion.weight"].shape[0]
                    else:
                        raise ValueError(f"Module {key} should contain to_v or to_k.")
                    attn_processors[key] = CustomDiffusionAttnProcessor(
                        train_k=train_k,
                        train_v=train_v,
                        train_q=train_q,
        ...
        **kwargs,
        ...
        if is_custom_diffusion:
            model_to_save = AttnProcsLayers(
                {
                    y: x
                    for (y, x) in self.unet.attn_processors.items()
                }
            )
            ...
            model_to_save = AttnProcsLayers(self.unet.attn_processors)
            state_dict = model_to_save.state_dict()
